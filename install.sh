...
nix-shell -p git --command "git clone https://github.com/VPuljic/nixos $SCRIPT_DIR"
...
sed -i "0,/vp/s//$(whoami)/" $SCRIPT_DIR/flake.nix
sed -i "0,/vp/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
sed -i "s/vedran.puljic1@gmail.com//" $SCRIPT_DIR/flake.nix
